...
bool MqttShowSensor(bool call_show_sensor)
{
  ...
  if (TasmotaGlobal.global_update && Settings->flag.mqtt_add_global_info) {
    if ((TasmotaGlobal.humidity > 0) || !isnan(TasmotaGlobal.temperature_celsius) || (TasmotaGlobal.pressure_hpa != 0)) {
      ...
      if (2 == add_comma) {
        float abs_humidity = CalcAbsHum(TasmotaGlobal.temperature_celsius, TasmotaGlobal.humidity);
        ResponseAppend_P(PSTR("%s\"" D_JSON_ABSOLUTE_HUMIDITY "\":%*_f"),
          (add_comma)?",":"", Settings->flag2.humidity_resolution, &abs_humidity);
